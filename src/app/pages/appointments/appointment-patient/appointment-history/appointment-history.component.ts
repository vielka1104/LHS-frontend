...
export class AppointmentHistoryComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'dni', 'doctor', 'fecha', 'puntuacion'];
  urlid!: number;
    ...
    this.appointmentservice
      ...
      .subscribe((response: any) => {
        ...
        for (var oneappointment of this.dataSource.data) {
          ...
          let dateformatselected = formatDate(
            oneappointment.scheduledAt,
            'YYYY-MM-dd HH:mm:ss',
            'en_US'
          );
  ...
  GotoBookAppointmentsPatient() {
    this.route.navigate([
      `patient/${this.patientobject.id}/appointment-patient`,
    ]);
  }
